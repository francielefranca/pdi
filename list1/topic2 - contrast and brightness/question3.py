...
contrast_factor = 1.25  # Ajuste conforme necessário
brightness_factor = 50  # Ajuste conforme necessário
adjusted_gato = adjust_contrast_brightness(gato, contrast_factor, brightness_factor)
show_results(gato, adjusted_gato, 'Imagem Original', 'Imagem Ajustada')

recife = load_img(nome_arquivo_imagem_recife)
#recife_gray = convert_to_grayscale(recife)
contrast_factor_rc = 0.75  # Ajuste conforme necessário
brightness_factor_rc = 12  # Ajuste conforme necessário
adjusted_recife = adjust_contrast_brightness(recife, contrast_factor_rc, brightness_factor_rc)
show_results(recife, adjusted_recife, 'Imagem Original', 'Imagem Ajustada')

upe = load_img(nome_arquivo_imagem_upe)
#upe_gray = convert_to_grayscale(upe)
contrast_factor_upe = 2  # Ajuste conforme necessário
brightness_factor_upe = 25  # Ajuste conforme necessário
adjusted_upe = adjust_contrast_brightness(upe, contrast_factor_upe, brightness_factor_upe)
show_results(upe, adjusted_upe, 'Imagem Original', 'Imagem Ajustada')

# solucao 3 - ajustando brilho/contraste direto (DONE - upe ainda melhoria mais)